...
klim = [1e9, -1e9]
epslim = [1e9, -1e9]
divdev_r_lim = [[1e9, -1e9], [1e9, -1e9]]
ulim = [[1e9, -1e9], [1e9, -1e9]]
...
for i0 in range(1):
    ...
    # Plot vertical dotted lines to indicate where the lines are samples
    epsilon_plot.axes.plot((-1, -1), (min(list_y[0]), max(list_y[0])), color=epsilon_plot.gray, alpha=.5, ls=':')
    epsilon_plot.axes.plot((1, 1), (min(list_y[0]), max(list_y[0])), color=epsilon_plot.gray, alpha=.5, ls=':')
    epsilon_plot.axes.plot((3, 3), (min(list_y[0]), max(list_y[0])), color=epsilon_plot.gray, alpha=.5, ls=':')
